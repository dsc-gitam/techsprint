    ...
    if (user != null) {
      ...
      getDocs(query(refTeam, where("participants", "array-contains", user.uid)))
        .then(async (snapshot) => {
          ...
          const dt = snapshot.docs[0].data()["milestone_four_time"];
  ...
  return (
    <>
      <div className="p-4 h-[88%] pt-16">
        <h3 className="text-xl font-bold uppercase opacity-50">Milestone 4</h3>
          ...
          onSubmit={async (f) => {
            ...
            if (teamDocId != undefined) {
              await updateDoc(doc(db, "teams", teamDocId), {
                milestone_four_link: dUrl,
                milestone_four_score_aggregate: -1,
                milestone_four_time: serverTimestamp(),
                milestone_four_score: [],
              });
              setLoader(false);
