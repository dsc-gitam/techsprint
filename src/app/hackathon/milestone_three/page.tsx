    ...
    if (user != null) {
      ...
      getDocs(query(refTeam, where("participants", "array-contains", user.uid)))
        .then(async (snapshot) => {
          ...
          const dt = snapshot.docs[0].data()["milestone_three_time"];
  ...
  return (
    <>
      <div className="p-4 h-[88%] pt-16">
        <h3 className="text-xl font-bold uppercase opacity-50">Milestone 3</h3>
        <h2 className="text-4xl font-medium mb-8">
          Progress Update / Testing Report
        </h2>
        <form
          onSubmit={async (f) => {
            f.preventDefault();
            ...
            if (teamDocId != undefined) {
              await updateDoc(doc(db, "teams", teamDocId), {
                milestone_three_link: dUrl,
                milestone_three_score_aggregate: -1,
                milestone_three_time: serverTimestamp(),
                milestone_three_score: [],
              });
              setLoader(false);
